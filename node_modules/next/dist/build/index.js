...
sprPages.forEach(page=>{if((0,_utils.isDynamicRoute)(page)){tbdPrerenderRoutes.push(page);delete defaultMap[page];}});// Append the "well-known" routes we should prerender for, e.g. blog
// post slugs.
additionalSprPaths.forEach((routes,page)=>{routes.forEach(route=>{defaultMap[route]={page};});});return defaultMap;},exportTrailingSlash:false};await exportApp(dir,exportOptions,exportConfig);// remove server bundles that were exported
for(const page of staticPages){const{serverBundle}=pageInfos.get(page);await fsUnlink(serverBundle);}const moveExportedPage=async(page,file,isSpr,ext)=>{file=`${file}.${ext}`;const orig=_path.default.join(exportOptions.outdir,file);const relativeDest=(isLikeServerless?_path.default.join('pages',file):_path.default.join('static',buildId,'pages',file)).replace(/\\/g,'/');const dest=_path.default.join(distDir,isLikeServerless?_constants2.SERVERLESS_DIRECTORY:_constants2.SERVER_DIRECTORY,relativeDest);if(!isSpr){pagesManifest[page]=relativeDest;if(page==='/')pagesManifest['/index']=relativeDest;if(page==='/.amp')pagesManifest['/index.amp']=relativeDest;}await mkdirp(_path.default.dirname(dest));await fsMove(orig,dest);};for(const page of combinedPages){const isSpr=sprPages.has(page);const isDynamic=(0,_utils.isDynamicRoute)(page);let file=page==='/'?'/index':page;// The dynamic version of SPR pages are not prerendered. Below, we handle
// the specific prerenders of these.
if(!(isSpr&&isDynamic)){await moveExportedPage(page,file,isSpr,'html');}const hasAmp=hybridAmpPages.has(page);if(hasAmp){await moveExportedPage(`${page}.amp`,`${file}.amp`,isSpr,'html');}if(isSpr){// For a non-dynamic SPR page, we must copy its data file from export.
if(!isDynamic){await moveExportedPage(page,file,true,'json');finalPrerenderRoutes[page]={initialRevalidateSeconds:exportConfig.initialPageRevalidationMap[page],srcRoute:null,dataRoute:_path.default.posix.join('/_next/data',`${page==='/'?'/index':page}.json`)};}else{// For a dynamic SPR page, we did not copy its html nor data exports.
// Instead, we must copy specific versions of this page as defined by
// `unstable_getStaticParams` (additionalSprPaths).
const extraRoutes=additionalSprPaths.get(page)||[];for(const route of extraRoutes){await moveExportedPage(route,route,true,'html');await moveExportedPage(route,route,true,'json');finalPrerenderRoutes[route]={initialRevalidateSeconds:exportConfig.initialPageRevalidationMap[route],srcRoute:page,dataRoute:_path.default.posix.join('/_next/data',`${route==='/'?'/index':route}.json`)};}}}}// remove temporary export folder
await(0,_recursiveDelete.recursiveDelete)(exportOptions.outdir);await fsRmdir(exportOptions.outdir);await fsWriteFile(manifestPath,JSON.stringify(pagesManifest),'utf8');}if(postBuildSpinner)postBuildSpinner.stopAndPersist();console.log();const analysisEnd=process.hrtime(analysisBegin);backgroundWork.push((0,_events.recordBuildOptimize)({durationInSeconds:analysisEnd[0],totalPageCount:pagePaths.length,staticPageCount:staticPages.size,ssrPageCount:pagePaths.length-staticPages.size}));if(sprPages.size>0){const finalDynamicRoutes={};tbdPrerenderRoutes.forEach(tbdRoute=>{const dataRoute=_path.default.posix.join('/_next/data',`${tbdRoute==='/'?'/index':tbdRoute}.json`);finalDynamicRoutes[tbdRoute]={routeRegex:(0,_utils.getRouteRegex)(tbdRoute).re.source,dataRoute,dataRouteRegex:(0,_utils.getRouteRegex)(dataRoute.replace(/\.json$/,'')).re.source.replace(/\(\?:\\\/\)\?\$$/,'\\.json$')};});const prerenderManifest={version:1,routes:finalPrerenderRoutes,dynamicRoutes:finalDynamicRoutes};await fsWriteFile(_path.default.join(distDir,_constants2.PRERENDER_MANIFEST),JSON.stringify(prerenderManifest),'utf8');}staticPages.forEach(pg=>allStaticPages.add(pg));pageInfos.forEach((info,key)=>{allPageInfos.set(key,info);});(0,_utils2.printTreeView)(Object.keys(mappedPages),allPageInfos,isLikeServerless);if(tracer){const parsedResults=await tracer.profiler.stopProfiling();await new Promise(resolve=>{if(parsedResults===undefined){tracer.profiler.destroy();tracer.trace.flush();tracer.end(resolve);return;}const cpuStartTime=parsedResults.profile.startTime;const cpuEndTime=parsedResults.profile.endTime;tracer.trace.completeEvent({name:'TaskQueueManager::ProcessTaskFromWorkQueue',id:++tracer.counter,cat:['toplevel'],ts:cpuStartTime,args:{src_file:'../../ipc/ipc_moji_bootstrap.cc',src_func:'Accept'}});tracer.trace.completeEvent({name:'EvaluateScript',id:++tracer.counter,cat:['devtools.timeline'],ts:cpuStartTime,dur:cpuEndTime-cpuStartTime,args:{data:{url:'webpack',lineNumber:1,columnNumber:1,frame:'0xFFF'}}});tracer.trace.instantEvent({name:'CpuProfile',id:++tracer.counter,cat:['disabled-by-default-devtools.timeline'],ts:cpuEndTime,args:{data:{cpuProfile:parsedResults.profile}}});tracer.profiler.destroy();tracer.trace.flush();tracer.end(resolve);});}await Promise.all(backgroundWork).catch(()=>{});}